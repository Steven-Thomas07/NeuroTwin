...
with col1:
    ...
    if st.button("Export Report as PDF"):
        ...
        pdf.cell(200, 10, txt="NeuroTwin Report", ln=1, align='C')
        pdf.cell(200, 10, txt=f"Depression Risk: {st.session_state.risk}%", ln=1)
        pdf.cell(200, 10, txt=f"Date: {pd.Timestamp.now().strftime('%Y-%m-%d')}", ln=1)
        pdf.output("NeuroTwin_Report.pdf")
        st.success("Report saved as NeuroTwin_Report.pdf")
