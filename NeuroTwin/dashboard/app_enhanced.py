...
if len(df) > 1:
    trend_df = df.copy()
    trend_df['risk'] = trend_df.apply(lambda row: np.clip(40 + row['stress']*6.2 - row['sleep_hours']*3.8, 0, 100), axis=1)
    st.line_chart(trend_df.set_index('date')['risk'])
    st.caption("7-Day Risk Trend")
...
if st.button("Export Full Report (PDF + 3D Brain)"):
    ...
    pdf.cell(0, 10, text="NeuroTwin Report", new_x=XPos.LMARGIN, new_y=YPos.NEXT, align='C')
    pdf.cell(0, 10, text=f"Risk: {risk}%", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
    pdf.image("brain.png", w=180)
    pdf.output("NeuroTwin_Report.pdf")
    st.success("Report with 3D brain saved!")
